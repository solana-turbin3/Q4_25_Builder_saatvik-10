...
pub const HOUSE_EDGE: u16 = 150; //1.5% of winnings
...
impl<'info> ResolveBet<'info> {
    pub fn verify_ed25519_signature(&mut self, sign: &[u8]) -> Result<()> {
        ...
        let sigs = Ed25519InstructionSignatures::unpack(&ix.data).unwrap().0;

        require_eq!(sigs.len(), 1, DiceError::Ed25519DataLength);

        let sig = &sigs[0];

        require!(sig.is_verifiable, DiceError::Ed25519Header);

        require_keys_eq!(
            sig.public_key.ok_or(DiceError::Ed25519Pubkey)?,
            self.house.key(),
            DiceError::Ed25519Pubkey
        );

        require!(
            &sig.signature.ok_or(DiceError::Ed25519Signature)?.eq(sign),
    ...
    pub fn resolve_bet(&mut self, bumps: &ResolveBetBumps, sign: &[u8]) -> Result<()> {
        let hash = hash(sign).to_bytes();
