...
export class TapShield {
  ...
  async recordClaim(
    ...
    const [faucetRegistry] = PublicKey.findProgramAddressSync([
      Buffer.from("faucet"),
      this.faucetKeypair.publicKey.toBuffer()
    ],
      this.program.programId
    )
    ...
    try {
      await this.program.methods.recordClaim(claimerPubkey, new BN(amount), new BN(cooldownSeconds))
        .accounts({
          operator: this.faucetKeypair.publicKey,
      ...
      if (err.message?.includes("ClaimTooRecent") || err.logs?.some((log: string) => log.includes("ClaimTooRecent"))) {
  ...
  async getClaimHistory(claimerPubkey: PublicKey): Promise<ClaimRecordInfo[]> {
    const claimRecords = await this.program.account.claimRecord.all([
      {
        memcmp: {
          offset: 8,
    ...
    return claimRecords.map((record: any) => ({
      ...
      faucetId: record.account.faucerId.toBase58(),
